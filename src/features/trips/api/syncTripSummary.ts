...
export const syncTripSummary = async (tripId: string): Promise<void> => {
  try {
    ...
    const costs = costsSnap.docs.map(
      (d: any) => ({ id: d.id, ...d.data() }) as Cost,
    );
    const participants = participantsSnap.docs.map(
      (d: any) => ({ id: d.id, ...d.data() }) as Participant,
    );

    // 1. Decision Hub: Propuestas que requieren votación y no están resueltas
    const activeProposals = proposals.filter(
      (p) => p.requiresVoting && p.status === "pending",
    );
    ...
    const proposalsByCategory = {
      ...
      logistics: activeProposals.filter(
        (p) => p.type === "accommodation" || p.type === "transport",
      ).length,
    ...
    const getStatus = (type: string): DashboardStatus => {
      ...
      if (typeProposals.some((p) => p.status === "confirmed"))
        return "confirmed";
      return "pending";
    };
    ...
    const totalBudget =
      tripData.budget ||
      proposals.reduce((acc, p) => acc + (p.estimatedCost || 0), 0);
