...
export function ActivityList({ tripId, currentUserId, isAdmin, onEdit }: ActivityListProps) {
  const { data: items = [], isLoading } = useProposals(tripId, "activity");
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center bg-white rounded-2xl shadow-sm border border-slate-100">
        <div className="w-16 h-16 bg-purple-50 rounded-full flex items-center justify-center mb-4">
          <Icon name="local_activity" className="w-8 h-8 text-purple-500" />
        </div>
        <h3 className="text-lg font-bold text-slate-700 mb-2">
          Nada en actividades aún
        </h3>
        <p className="text-slate-500 text-sm max-w-sm">
          Las actividades confirmadas aparecerán aquí.
        </p>
      </div>
    );
  }
