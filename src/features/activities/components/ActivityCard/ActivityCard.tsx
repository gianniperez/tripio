...
export function ActivityCard({ item, canManage, onEdit, onDelete, userProfiles }: ActivityCardProps) {
  const [showActions, setShowActions] = useState(false);
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);

  return (
    <NeumorphicCard className="mb-4 relative overflow-hidden transition-all duration-300 hover:scale-[1.01] border-l-4 border-l-purple-500/30">
      <div className="absolute top-2 right-2 flex items-center gap-2 z-20">
        {canManage && (
          <div className="relative">
            <button
              onClick={() => setShowActions(!showActions)}
              className="p-1.5 text-slate-400 hover:text-purple-500 hover:bg-slate-100 rounded-full transition-all cursor-pointer"
            >
      ...
      <div className="flex justify-between items-start mb-2">
        <div>
          <span className="text-[10px] font-bold uppercase tracking-widest text-purple-500 mb-1 opacity-80 flex items-center gap-1">
            <Icon name="local_activity" className="w-3 h-3" /> Actividad
          </span>
