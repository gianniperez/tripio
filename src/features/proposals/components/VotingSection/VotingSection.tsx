...
export function VotingSection<T extends BaseProposalFields>({
  register,
  control,
  watch,
  errors,
  isProposalMode = true,
}: VotingSectionProps<T>) {
  const { fields, append, remove } = useFieldArray({
    control,
    name: "options" as any,
  });

  const requiresVoting = watch("requiresVoting" as any);
  const responseType = watch("responseType" as any);

  const showOptions = isProposalMode || requiresVoting;

  return (
    <div className="space-y-4">
      {/* Voting Requirements Toggle - Hidden in Proposal Mode because it's mandatory */}
      {!isProposalMode && (
        <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 shadow-neumorphic-inset-sm animate-in fade-in duration-300">
          <div className="flex items-center justify-between">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                {...register("requiresVoting" as any)}
