...
export const ProposalForm = ({
  ...
  const submitWrapper = (data: CreateProposalFormValues) => {
    ...
    if (
      ...
      (typeof cleanedData.estimatedCost === "string" &&
        (cleanedData.estimatedCost as string).trim() === "")
    ) {
      cleanedData.estimatedCost = null;
    }
    ...
    const normalizeToMidnight = (
      d: Date | null | undefined,
      isUtcMidnight: boolean = false,
    ) => {
    ...
    if (cleanedData.startDate) {
      ...
      const proposalStart = normalizeToMidnight(
        proposalStartNode,
        isAccommodation,
      );
    ...
    if (cleanedData.endDate) {
      ...
      const proposalStartNode = cleanedData.startDate
        ? new Date(cleanedData.startDate)
        : null;
      const proposalStart = proposalStartNode
        ? normalizeToMidnight(proposalStartNode, isAccommodation)
        : null;
      const tEnd = tripEnd
        ? normalizeToMidnight(tripEnd, isAccommodation)
        : null;
    ...
    if (isAccommodation && cleanedData.startDate && cleanedData.endDate) {
      ...
      if (newStart && newEnd) {
        const overlappingAccommodation = allProposals.find((p) => {
          if (p.type !== "accommodation" || p.status === "rejected")
            return false;
          ...
          return (
            newStart.getTime() < eEnd.getTime() &&
            newEnd.getTime() > eStart.getTime()
          );
        });
        ...
        if (overlappingAccommodation) {
          ...
          setError("endDate", {
            type: "manual",
            message:
              "Selecciona un rango de fechas que no coincida con otros alojamientos confirmados",
          });
          return;
        }
      }
  ...
  return (
    <div className="w-full">
      ...
      >
        <div
          className={`grid ${allowedTypes && allowedTypes.length === 1 ? "grid-cols-1" : "grid-cols-2"} gap-4 items-end`}
        >
          {(!allowedTypes || allowedTypes.length > 1) && (
            <div className="col-span-1">
              <label className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                <Icon name="add" className="w-3 h-3" /> Tipo
              </label>
              <select
                {...register("type")}
          ...
          >
            <label className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
              <Icon name="description" className="w-3 h-3" /> Título
            </label>
            <NeumorphicInput
              {...register("title")}
        ...
        <div>
          <label className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
            <Icon name="notes" className="w-3 h-3" /> Descripción (opcional)
          </label>
          <textarea
        ...
        {isInventory && (
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-1">
              <label className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                <Icon name="person" className="w-3 h-3 text-primary/70" />{" "}
                Asignado a (Opcional)
              </label>
              <select
                {...register("assignedTo")}
            ...
            <div className="col-span-1">
              <label className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                <Icon name="add" className="w-3 h-3 text-primary/70" /> Cantidad
              </label>
              <NeumorphicInput
                type="number"
        ...
        {isInventory && (
          <div className="grid grid-cols-1 gap-4">
            <div>
              <label className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                <Icon name="package_2" className="w-3 h-3 text-primary/70" />{" "}
            ...
            {!watch("isPersonal") && (
              <div className="flex items-center gap-2 mt-2">
                <label className="flex items-center gap-2 text-xs font-medium text-slate-500 cursor-pointer">
                  ...
                  <ContextualInfo description="Los ítems personales no son visibles para el resto del grupo y no se incluyen en la planificación colectiva." />
                </label>
              </div>
            )}
          </div>
        ...
        {isTransport && (
          <div className="grid grid-cols-2 gap-4">
            ...
            {isPersonalTransport && (
              <div className="col-span-1">
                <label className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                  <Icon name="add" className="w-3 h-3 text-primary/70" />{" "}
                  Capacidad (Personas)
                </label>
                <NeumorphicInput
                  type="number"
        ...
        {(isActivity || isAccommodation || isTransport) && (
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-1">
              <label className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                <Icon name="map" className="w-3 h-3 text-primary/70" />{" "}
                {isAccommodation
                  ? "Dirección / Link"
                  : isTransport
                    ? "Punto de Salida / Ubicación"
                    : "Ubicación / Dirección"}
                <ContextualInfo description="Podés ingresar una dirección física o un link a Google Maps/Airbnb para que todos sepan cómo llegar." />
              </label>
              <NeumorphicInput
                {...register("location")}
                placeholder={
            ...
            <div className="col-span-1">
              <label className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                <Icon
                  name="attach_money"
                  className="w-3 h-3 text-green-500/70"
                />{" "}
        ...
        {isActivity && (
          <div className="grid grid-cols-1 gap-4">
            <div>
              <label className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                <Icon name="calendar_month" className="w-3 h-3" /> Fecha y Hora
              </label>
              <input
                type="datetime-local"
        ...
        {(isAccommodation || isTransport) && (
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                <Icon name="calendar_month" className="w-3 h-3" />{" "}
            ...
            <div>
              ...
              <input
                ...
                min={
                  safeToLocalISO(watchedStartDate, !isAccommodation) ||
                  floorDateSimple
                }
