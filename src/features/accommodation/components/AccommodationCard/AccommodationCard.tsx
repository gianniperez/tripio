import type { AccommodationCardProps } from "./AccommodationCard.types";
...
export function AccommodationCard({ item, canManage, onEdit, onDelete, userProfiles }: AccommodationCardProps) {
  const [showActions, setShowActions] = useState(false);
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);

  return (
    <NeumorphicCard className="mb-4 relative overflow-hidden transition-all duration-300 hover:scale-[1.01] border-l-4 border-l-blue-500/30">
      <div className="absolute top-2 right-2 flex items-center gap-2 z-20">
        {canManage && (
          <div className="relative">
            <button
              onClick={() => setShowActions(!showActions)}
              className="p-1.5 text-slate-400 hover:text-blue-500 hover:bg-slate-100 rounded-full transition-all cursor-pointer"
            >
      ...
      <div className="flex justify-between items-start mb-2">
        <div>
          <span className="text-[10px] font-bold uppercase tracking-widest text-blue-500 mb-1 opacity-80 flex items-center gap-1">
            <Icon name="bed" className="w-3 h-3" /> Alojamiento
          </span>
      ...
      <div className="grid grid-cols-2 gap-3 mt-4 text-xs text-slate-500 font-medium bg-slate-50/50 p-3 rounded-xl border border-slate-100/50">
        {item.startDate && (
          <div className="flex items-center">
            ...
            <span className="truncate">
              {format(item.startDate.toDate(), "d MMM", { locale: es })}
              {item.endDate &&
                ` - ${format(item.endDate.toDate(), "d MMM", { locale: es })}`}
            </span>
          </div>
        )}
        {item.location && (
        ...
        {item.estimatedCost && (
          <div className="flex items-center">
            ...
            />
            <span>${item.estimatedCost} total</span>
          </div>
        )}
      </div>
