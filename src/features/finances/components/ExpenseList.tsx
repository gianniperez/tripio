...
import { Calendar, CheckSquare, MessageSquare, Trash2, Loader2 } from 'lucide-react';
...
export const ExpenseList = ({ tripId, currentUserId, costs, events, proposals, currency }: ExpenseListProps) => {
  ...
  if (costs.length === 0) {
    return (
      <div className="bg-surface rounded-tripio p-8 shadow-neumorphic text-center flex flex-col items-center justify-center gap-3 mt-4">
        ...
        <div>
          <p className="text-gray-600 font-bold mb-1">No tienes gastos proyectados</p>
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3 overflow-hidden">
                <div className={`mt-0.5 p-2 rounded-lg shrink-0 ${isProposal ? 'bg-secondary/10' : 'bg-primary/10'}`}>
                  {sourceIcon}
                </div>
                <div className="min-w-0 pr-2">
                  <p className="font-bold text-gray-700 leading-tight truncate">{cost.description}</p>
                  <p className="text-xs text-gray-500 mt-1 flex items-center gap-1.5 font-medium truncate">
                    <span className="truncate">{sourceName}</span>
                    {isProposal && <span className="bg-secondary/10 text-secondary px-1.5 rounded-sm text-[10px] font-bold shrink-0">Propuesta</span>}
                  </p>
                </div>
              </div>
              <div className="text-right flex items-center justify-end gap-3 ml-2 shrink-0">
                ...
                {!isProposal && !cost.linkedEventId && cost.createdBy === currentUserId && (
                  <button
