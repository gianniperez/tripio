...
export const DailyBudgetCard = ({
    ...
    return (
      <NeumorphicCard className="p-4 flex items-center gap-3 w-full">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400 truncate">Define fechas del viaje</p>
        </div>
      </NeumorphicCard>
    );
  }

  const days = Math.max(1, differenceInDays(endDate.toDate(), startDate.toDate()) + 1);
  ...
  return (
    <NeumorphicCard className="p-4 flex items-center gap-4 w-full">
        <div className="w-12 h-12 rounded-xl bg-teal-100 flex items-center justify-center shrink-0 shadow-neumorphic-inset">
          <Wallet className="w-6 h-6 text-teal-600" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-xs text-gray-500 font-bold mb-0.5">Sugerido por día</p>
